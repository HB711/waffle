...
Delta = 0.60         # local box half-width around x_loc (you used 0.60 in log)
...
# Orthogonality penalty schedule:
WARMUP = 100
ALPHA_RAMP_STEPS = 200
ALPHA_MAX = 50
EPS_OVLP = 1e-8
OVLP_SUBSAMPLE = 4000
...
#   This matches your earlier "tempered" pattern: model_j is always no_grad.
# ============================================================
...
def alpha_schedule(it: int) -> float:
    ...
    t = (it - WARMUP) / float(ALPHA_RAMP_STEPS)
    t = max(0.0, min(1.0, t))
    return float(ALPHA_MAX * t)

# ============================================================
# Training (MH only; penalty only for state1; and penalty grad does NOT touch state0)
# ============================================================
def train_vmc_mh_local_simple(steps=N_steps, eta=lr):
    ...
    for it in range(steps):
        ...
        # IMPORTANT: penalty is applied ONLY to state1 (model_1), and gradient flows ONLY to model_1.
        x0s = subsample_pack(xs_pack[0], OVLP_SUBSAMPLE)
        x1s = subsample_pack(xs_pack[1], OVLP_SUBSAMPLE)

        if (alpha > 0.0) or (it % PRINT_EVERY == 0):
        ...
        for i in range(N_states):
            ...
            if (i == 1) and (alpha > 0.0) and (pen01_t is not None):
        ...
        if it == 0 or (it % PRINT_EVERY) == 0:
            phase = "warmup" if it < WARMUP else ("ramp" if it < (WARMUP + ALPHA_RAMP_STEPS) else "fixed")
