...
# Warmup
WARMUP = 200  # first 300 steps: no orth penalties at all

# Pairwise alpha_ij adaptation (after warmup)
S_TRIGGER = 1e-3         # if S_ij >= 0.001 => increase alpha_ij
ALPHA_INIT = 1.0         # initial alpha_ij after warmup (small but nonzero)
ALPHA_UP = 2.0           # multiplicative increase (fast)
ALPHA_DOWN = 0.9        # multiplicative decay when S is already small
ALPHA_MAX = 1e7          # safety clamp
ALPHA_MIN = 0.0          # allow decay to 0 if you want
...
PRINT_EVERY = 10
OVLP_EVERY  = 50
OVLP_SUBSAMPLE = 512  # for overlap matrix printing + alpha updates (faster)
...
# ============================================================
# Printing overlap as matrix (nice)
# ============================================================
@torch.no_grad()
...
def adapt_alpha_pairwise(alpha_mat, S_mat, it):
    """
    After warmup: if S_ij >= S_TRIGGER => alpha_ij *= ALPHA_UP (fast)
                 else alpha_ij *= ALPHA_DOWN (slow decay)
    Only for i>j.
    """
    if it < WARMUP:
        return alpha_mat
...
def train_vmc_ex(n_states=N_states, steps=N_steps, eta=lr):
    ...
    for it in range(steps):
        ...
        # ---- compute overlaps (subsample) for alpha update + printing ----
        # do it every OVLP_EVERY steps to save time, but alpha adapts only when we compute S_mat
        S_mat = None
        if (it % OVLP_EVERY) == 0:
        ...
        for i in range(n_states):
            ...
            E_real, _, f, _ = local_energy_5D(x2,x3,x4,x5,x6, models[i])
            li = energy_loss_from_real_local_energy(E_real, f)
            ...
            if it >= WARMUP:
                for j in range(i):
                    ...
                    if aij <= 0.0:
                        continue
                    # penalty computation uses full xs (you can also subsample here if too slow)
                    Sabs, pen = compute_Sabs_and_penalty_for_pair(models[i], xs_it[i], models[j], xs_it[j])
                    li = li + aij * pen
        ...
        if S_mat is not None:
            ...
            # also print current alpha_ij matrix (lower triangle) for quick check
            # show as full matrix with zeros on diag/upper
