...
# ============================================================
# Hyperparameters (MH only, local box; penalty affects ONLY state1)
# ============================================================
# Sampling / MH
N_walkers = 4000
N_burn_in = 100
Sigma0 = 0.30
P_GLOBAL = 0.02      # "global" jump uniformly inside local box
Delta = 0.60         # local box half-width around x_loc
...
# Adaptive orthogonality penalty
ALPHA_INIT = 1.0
ALPHA_MIN = 1e-3
ALPHA_MAX = 200.0

ALPHA_ADJUST_EVERY = 50
OVLP_HIGH = 0.10
OVLP_LOW  = 0.01
ALPHA_GROWTH = 1.5
ALPHA_SHRINK = 0.7
...
# ============================================================
# Direct phase embedding
# ============================================================
def phase_emb(x2, x3, x4, x5, x6):
    return torch.stack([x2, x3, x4, x5, x6], dim=-1)  # (N,5)

# ============================================================
# Model
# ============================================================
class NN_Waffle_Complex_5D(nn.Module):
    def __init__(self, input_dim=5, hidden_dim=Hidden_Dim):
        super().__init__()
...
def mh_chain_local(model: nn.Module, state, x_loc: torch.Tensor, sigma: float, Nb: int, p_global: float, delta: float):
    ...
    for _ in range(Nb):
        if torch.rand((), device=device) < p_global:
            ...
            nx2 = wrap_centered_1d(x2 + torch.randn_like(x2) * sigma, x_loc[0])
            nx3 = wrap_centered_1d(x3 + torch.randn_like(x3) * sigma, x_loc[1])
            nx4 = wrap_centered_1d(x4 + torch.randn_like(x4) * sigma, x_loc[2])
            nx5 = wrap_centered_1d(x5 + torch.randn_like(x5) * sigma, x_loc[3])
            nx6 = wrap_centered_1d(x6 + torch.randn_like(x6) * sigma, x_loc[4])
...
def overlap_abs_symmetric_stable_oneway(model_i, model_j, x_i, x_j, eps=EPS_OVLP):
    """
    Symmetric |S_ij| estimator:
      A = <exp((f_j-f_i) + i(g_j-g_i))>_{x~pi_i}
      B = <exp((f_i-f_j) + i(g_i-g_j))>_{x~pi_j}
      |S| = sqrt(|A||B|)
    BUT: gradient flows ONLY to model_i; model_j is evaluated under no_grad always.
    """
...
# ============================================================
def update_alpha_from_overlap(alpha, Sabs):
    """
    Update alpha every ALPHA_ADJUST_EVERY steps:
      - if |S01| > OVLP_HIGH, grow alpha
      - if |S01| < OVLP_LOW,  shrink alpha
      - otherwise keep alpha
    """
    if Sabs > OVLP_HIGH:
        new_alpha = min(ALPHA_MAX, alpha * ALPHA_GROWTH)
        action = "grow"
    elif Sabs < OVLP_LOW:
        new_alpha = max(ALPHA_MIN, alpha * ALPHA_SHRINK)
        action = "shrink"
    else:
        new_alpha = alpha
        action = "keep"
    return float(new_alpha), action

# ============================================================
# Training
#   - MH only
#   - penalty only for state1
#   - penalty grad does NOT touch state0
#   - alpha adjusted every 50 steps based on |S01|
# ============================================================
def train_vmc_mh_local_simple(steps=N_steps, eta=lr):
    assert N_states == 2, "This simplified trainer is for N_states=2."
    ...
    def subsample_pack(xpack, n=OVLP_SUBSAMPLE):
        if (n is None) or (xpack[0].shape[0] <= n):
            return xpack
        idx = torch.randperm(xpack[0].shape[0], device=xpack[0].device)[:n]
        return tuple(t[idx] for t in xpack)
    ...
    for it in range(steps):
        ...
        # Adjust alpha every ALPHA_ADJUST_EVERY steps, then hold it fixed
        if (it > 0) and (it % ALPHA_ADJUST_EVERY == 0):
            alpha, alpha_action = update_alpha_from_overlap(alpha, last_S01)
        else:
            alpha_action = "hold"
        ...
        if it == 0 or (it % PRINT_EVERY) == 0:
            ...
            print(
                f"[it={it}] alpha={alpha:.3e} alpha_action={alpha_action} "
                f"|S01|={last_S01:.3e} pen={last_pen:.3e} "
                f"adj_every={ALPHA_ADJUST_EVERY} ovlp_low={OVLP_LOW:.2e} ovlp_high={OVLP_HIGH:.2e} "
