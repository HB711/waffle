...
Sigma0 = 0.30
P_GLOBAL = 0.02
Delta = 1.2  # local box half-width
...
#   We estimate complex S and build a linear objective whose grad equals
#   2*alpha*Re(S* dS) under the "model0 frozen" formula.
# ============================================================
@torch.no_grad()
...
def penalty_obj_absS2_oneway_explicit(
    ...
    # d/dθ [ mean(w_f f + w_g g) ] = mean(w_f ∇f + w_g ∇g)
    cross_obj = (w_f * f_i_j + w_g * g_i_j).mean()

    # normalization term corresponds to -S * Re E_i[(∇log ψ_i)^*] => -|S|^2 * E_i[∇f]
    # We realize E_i[∇f] as gradient of mean(f_i_i)
...
#   - penalty is |S|^2
#   - explicit gradient descent (no Adam)
# ============================================================
def train_vmc_mh_local_adapt_alpha(steps=N_steps, eta=lr):
    ...
    for it in range(steps):
        ...
        # We'll build penalty objective during gradient computation below; but we also want |S| for logging / alpha update.
        # We'll compute a "no_grad overlap |S|" here using the same estimator as penalty uses (p0 samples).
        with torch.no_grad():
            # compute |S| using p0 (state0) samples only
            ...
            dg = (g1_j - g0_j)
            r_mean = _stable_mean_exp_complex(df, phase=(-dg))  # E0[O10^*]
            denom2 = _stable_mean_exp_real(2.0 * df)           # E0[|O10|^2]
        ...
        Sabs2_t = torch.tensor(last_S01_2, device=device, dtype=dtype)
        if (it >= WARMUP) and (alpha > 0.0):
            # penalty uses i=1 trainable, j=0 frozen
            pen_obj, Sabs2_t, _Sabs_t = penalty_obj_absS2_oneway_explicit(
                model_i=models[1],
                model_j=models[0],
