...
#             nn.SiLU(),
#         )
#         self.f_out = nn.Linear(hidden_dim, 1)  # log|psi|
#         self.g_out = nn.Linear(hidden_dim, 1)  # phase
...
@torch.no_grad()
def mh_chain_tempered(model: nn.Module, state, sigma: float, Nb: int, beta: float, p_global: float):
...
# IMPORTANT FIX: ramp trigger aligned by it%RAMP_EVERY, NOT (it-WARMUP)%...
# ============================================================
    ...
    for it in range(steps):
        ...
        for i in range(n_states):
            mh_states[i], acc = mh_chain_tempered(
                models[i], mh_states[i], sigma=sigmas[i], Nb=N_burn_in,
        ...
        if it == 0 or (it % PRINT_EVERY) == 0:
            ...
            print(f"[it={it}] phase={phase} beta={beta_it:.3f} p_global={P_GLOBAL:.2f} beta_imag={BETA_IMAG:.1e} | "
                  + " | ".join(parts))
